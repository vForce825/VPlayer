...
final class PlaybackClockTests: XCTestCase {
    ...
    func testAirPlayVideoPresentationOffsetUsesOnlyOutputLatency() {
        let airPlay = AudioOutputRouteSnapshot(
            category: .airPlay,
            reason: .initial,
            revision: 0,
            outputLatency: 0.450,
        ...
        XCTAssertEqual(
            AirPlayVideoPresentationOffsetPolicy.compute(for: airPlay).seconds,
            0.450,
            accuracy: 0.001
        )
        for category in [
            AudioOutputRouteCategory.hdmi,
            .bluetooth,
            .other,
            .none,
        ] {
            XCTAssertEqual(
                AirPlayVideoPresentationOffsetPolicy.compute(for: AudioOutputRouteSnapshot(
                    category: category,
                    reason: .initial,
                    revision: 0,
                    outputLatency: 0.450,
        ...
        XCTAssertEqual(AirPlayVideoPresentationOffsetPolicy.compute(for: nil), .zero)
    }

    func testAirPlayBoundaryRequiresReanchorEvenWhenReportedLatencyIsZero() {
        let airPlay = AudioOutputRouteSnapshot(
            category: .airPlay,
        ...
        let hdmi = AudioOutputRouteSnapshot(
            category: .hdmi,
            reason: .routeConfigurationChange,
            revision: 2,
            outputLatency: 0,
